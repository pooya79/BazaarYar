...
from server.features.agent.sandbox.sandbox_schema import SandboxExecutionResult
...
@pytest.mark.asyncio
async def test_python_sandbox_tool_accepts_attachment_ids_as_array(monkeypatch):
    settings = get_settings()
    monkeypatch.setattr(settings, "sandbox_tool_enabled", True)
    monkeypatch.setattr(settings, "sandbox_max_code_chars", 20_000)
    ...
    output = await sandbox_tool.run_python_analysis.ainvoke(
        {
            "code": "print('hello')",
            "attachment_ids_json": ["att-1", "att-2"],
        }
    )
