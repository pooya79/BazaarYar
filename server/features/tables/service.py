...
async def create_table(
    ...
    *,
    ...
    if await repo.get_table_by_name(session, name=table_name):
...
async def list_tables(session: AsyncSession, *, limit: int = 100, offset: int = 0) -> list[ReferenceTableSummary]:
    items = await repo.list_tables(session, limit=limit, offset=offset)
...
async def get_table(session: AsyncSession, *, table_id: UUID | str) -> ReferenceTableDetail:
    table = await repo.get_table(session, table_id=table_id, with_columns=True)
    return _table_to_detail(table)
...
async def delete_table(session: AsyncSession, *, table_id: UUID | str) -> None:
    table = await repo.get_table(session, table_id=table_id, with_columns=False)
    await repo.delete_table(session, table=table)
...
async def mutate_rows(
    ...
    *,
    ...
    inserted, updated, deleted = await repo.batch_mutate_rows(
        session,
        table=table,
...
async def start_import(
    ...
    *,
    ...
    attachment = await repo.get_attachment_for_import(session, attachment_id=attachment_id)
    ...
    try:
        ...
        job = await repo.finish_import_job(
            session,
            job=job,
            status=status,
...
async def get_import_job(
    ...
    *,
    ...
    job = await repo.get_import_job(session, table_id=table_id, job_id=job_id)
